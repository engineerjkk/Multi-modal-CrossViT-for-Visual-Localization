...
class MultiScaleBlock(nn.Module):
    ...
    def __init__(self, dim, patches, depth, num_heads, mlp_ratio, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        for d in range(num_branches):
            d_ = (d+1) % num_branches
            nh = num_heads[d_]
            if depth[-1] == 0:  # backward capability: #여기로 들어간다. 
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384), depth=([1, 3, 1], [1, 3, 1], [1, 3, 1]),
        ...
        if hybrid_backbone is None:
            ...
            param0 = param_list[0]
            param1 = param_list[1]
        else:#여기로 들어가지는 않는다. 그러니까 걱정할 필요없다. 순순히 Learnable parameter이다. 
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_branches):
            ...
            tmp = tmp + self.pos_embed[i]##여기서 patch에 해당하는 값이 더해지는거구나.
            param_list=self.pos_embed
            param0 = param_list[0]# <=여기에 1,1,192차원의 NGP 텐서가 들어가야 한다.
            param1 = param_list[1]# <=여기에 1,1,384차원의 NGP 텐서가 들어가야 한다.
