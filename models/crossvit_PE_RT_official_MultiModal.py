...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384), depth=([1, 3, 1], [1, 3, 1], [1, 3, 1]),
        ...
        param_list=self.pos_embed
        param0 = param_list[0]# <=여기에 1,1,192차원의 NGP 텐서가 들어가야 한다.
        param1 = param_list[1]# <=여기에 1,1,384차원의 NGP 텐서가 들어가야 한다.
    ...
    def forward_features(self, x, PosEnc):
        ...
        for i in range(self.num_branches):
            if i==0 :
                ...
                # print("1.tmp.shape",tmp.shape)
                # print("1.self.pos_embed[i].shape",self.pos_embed[i].shape)
                tmp = tmp + self.pos_embed[i]#1
                param_list=self.pos_embed
                param0 = param_list[0]# <=여기에 1,1,192차원의 NGP 텐서가 들어가야 한다.
                param1 = param_list[1]# <=여기에 1,1,384차원의 NGP 텐서가 들어가야 한다.
                ...
                #tmp = self.pos_drop(PosEnc)
                # print("2. tmp.shape : ",tmp.shape)
                # print("2. self.pos_embed[i].shape : ",self.pos_embed[i].shape)
                tmp = PosEnc + self.pos_embed[i]#2
                tmp = self.pos_drop(tmp)
                #print("2.tmp.shape",tmp.shape)
                xs.append(tmp)
    ...
    def forward(self, x,PosEnc):
        xs = self.forward_features(x,PosEnc)
